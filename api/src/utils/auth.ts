 ...
 *
 * Mirrors server/src/utils/auth.ts (the Hono/production backend) — see that
 * repo's own "two backends, one behaviour" convention in CLAUDE.md.
 */

import { HttpRequest } from '@azure/functions';

// Note: Uses VITE_-prefixed fallback for consistency with the frontend's
// SUITE_AUTH_URL default (Azure Static Web Apps historically shared env vars
// between frontend and API); SUITE_AUTH_URL itself is server-only.
 ...
 * folds into 'admin'); 'viewer' is read-only plus public tracking (always
 * anonymous, ungated by role).
 */
