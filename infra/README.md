...
| `APPLICATIONINSIGHTS_CONNECTION_STRING` | Application Insights connection string (from `modules/monitoring.bicep`), wired directly into `containerapps.bicep`'s container template (not via `seed-secrets.sh`). Read by `server/src/utils/appInsights.ts` at bootstrap — Container Apps' own log forwarding only reaches Log Analytics' `ContainerAppConsoleLogs_CL` table, not Application Insights' `traces` table, so the `applicationinsights` SDK is initialized explicitly to get console output (incl. the `METRIC ...` lines from `telemetryMetrics.ts`, used by Station Manager's cross-app weekly consumption summary) into `traces`. Unset ⇒ no-op, console logging still works locally/via `docker logs`. | bicep template output |
...
| `VIEWER_PINS_ENABLED` | `false` disables the opt-in "families waiting here" sharing on tracking pages (the navigator's waiting-spot heat map). Any other value (incl. unset) ⇒ enabled. Pins are memory-only, coarsened to ~110m, and never persisted. | `infra/.env.<env>` (optional) |
