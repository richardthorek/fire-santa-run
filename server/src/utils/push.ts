/**
 * Web Push for "notify me when Santa starts".
 *
 * Anonymous public viewers subscribe on the tracking page before a run; the
 * first location broadcast for a route triggers one push to every subscriber.
 * Configured via VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY / VAPID_SUBJECT — when
 * unset, the endpoints report unconfigured and the client hides the UI, so
 * push is an optional deployment feature (generate keys with
 ...
 * once per NOTIFY_WINDOW_MS (a `!meta` marker entity records the last send).
 * Fire-and-forget from the broadcast path — must never throw.
 */
export async function notifyRunStartOnce(routeId: string, appBaseUrl: string): Promise<void> {
  try {
    ...
    // Recency marker — skip when this run already got its start push.
    try {
    ...
    if (sends.length > 0) {
      console.log(`Sent run-start push to ${sends.length} subscriber(s) for route ${routeId}`);
    }
  } catch (error) {
